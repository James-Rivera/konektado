...
- Public feed cards do not need repeated verified badges by default, but trust must still be understandable in detail and decision contexts.
...
- Secondary CTA: "Save" only if saved items are connected or a clear disabled state exists.
- Trust signals needed: honest rating/review count, honest completed jobs count, barangay verification cue in the trust area if product confirms it should appear on detail pages, safe public location only, and no private phone/address/ID exposure.
...
- Secondary CTA: "Save" only if saved items are connected or intentionally disabled.
...
- Evidence from code/UI/source-of-truth: Job CTA is always rendered at `app/job/[jobId].tsx:323`; worker CTA is always rendered at `app/worker/[workerId].tsx:265`; `startJobConversation` blocks own jobs and non-open statuses at `services/conversation.service.ts:405` and `services/conversation.service.ts:409`; `startServiceConversation` blocks own services and services not accepting messages at `services/conversation.service.ts:466` and `services/conversation.service.ts:470`; docs say closed/cancelled jobs should not accept new interested workers in `docs/04-user-flows.md:119` and `docs/04-user-flows.md:141`.
...
- Affected files/components: `app/job/[jobId].tsx`, `app/worker/[workerId].tsx`, `services/conversation.service.ts`.
...
- Evidence from code/UI/source-of-truth: Worker Profile creates fallback completed jobs and hours at `app/worker/[workerId].tsx:147` and `app/worker/[workerId].tsx:150`; `getFallbackCount` is defined at `app/worker/[workerId].tsx:498`; service helper fallbacks return fake ratings/jobs at `services/marketplace.helpers.ts:323` and `services/marketplace.helpers.ts:331`; client helper fallbacks return fake ratings/jobs posted at `services/marketplace.helpers.ts:355` and `services/marketplace.helpers.ts:363`. Source docs position Konektado as trust-first in `docs/00-project-brief.md:19` and say ratings/reviews update from real review data in `docs/04-user-flows.md:172`.
...
- Affected files/components: `app/worker/[workerId].tsx`, `services/marketplace.helpers.ts`, `app/job/[jobId].tsx`, `app/(tabs)/index.tsx`, `app/(tabs)/search.tsx`, `components/home/HomeFeedCard.tsx`, `components/search/SearchJobResultCard.tsx`, `components/search/SearchWorkerResultCard.tsx`.
...
- Evidence from code/UI/source-of-truth: Job Details shows a Save alert at `app/job/[jobId].tsx:201`; Worker Profile shows a Save alert at `app/worker/[workerId].tsx:80`; Search Save uses placeholders at `app/(tabs)/search.tsx:161` and `app/(tabs)/search.tsx:173`; Home passes `undefined` for verified-user saves at `app/(tabs)/index.tsx:525` and `app/(tabs)/index.tsx:535`, while `HomeFeedCard` still renders the bookmark and stops propagation at `components/home/HomeFeedCard.tsx:78` and `components/home/HomeFeedCard.tsx:240`. Docs list saved providers/jobs as planned/stubbed in `docs/03-feature-map.md:38`.
...
- Affected files/components: `app/job/[jobId].tsx`, `app/worker/[workerId].tsx`, `app/(tabs)/index.tsx`, `app/(tabs)/search.tsx`, `components/home/HomeFeedCard.tsx`, `components/search/SearchJobResultCard.tsx`, `components/search/SearchWorkerResultCard.tsx`.
...
- Affected files/components: `app/(tabs)/search.tsx`, `components/search/SearchJobResultCard.tsx`, `components/search/SearchWorkerResultCard.tsx`, `app/worker/[workerId].tsx`.
- Risk level: Low.
...
- Evidence from code/UI/source-of-truth: Hero metrics show rating/jobs/availability at `app/worker/[workerId].tsx:165`; Worker details repeats location/jobs/availability/hours at `app/worker/[workerId].tsx:200`; services/rate are separate at `app/worker/[workerId].tsx:183`; service tags appear again at `app/worker/[workerId].tsx:224`; docs say worker details should be person-first and prioritize rating, jobs done, availability, services, and rate in `docs/03-feature-map.md:31`.
...
- Affected files/components: `app/worker/[workerId].tsx`.
...
- Evidence from code/UI/source-of-truth: Job budget is shown at `app/job/[jobId].tsx:251`; worker rate is shown at `app/worker/[workerId].tsx:193`; docs say no in-app payments and outside agreements in `docs/00-project-brief.md:88` and `docs/00-project-brief.md:98`; data model says budget is informational only in `docs/05-data-model.md:275`.
...
- Affected files/components: `app/job/[jobId].tsx`, `app/worker/[workerId].tsx`.
- Risk level: Low.
...
- Evidence from code/UI/source-of-truth: Copy is at `app/worker/[workerId].tsx:245`. Design rules require plain, user-readable labels in `docs/08-design-system.md:68`.
...
- Problem: `/worker/[workerId]` receives a service id, loads one active service, and then shows "Worker Profile".
- Evidence from code/UI/source-of-truth: Route param comment says the `workerId` is a service id at `app/worker/[workerId].tsx:40`; `getServiceDetail` fetches by service id at `services/service-profile.service.ts:203`; Search pushes `worker.id` where `worker.id` is service id at `app/(tabs)/search.tsx:169`; Worker Profile also shows "More services from this worker" at `app/worker/[workerId].tsx:251`.
...
- Recommended fix: Keep the route for now, but make copy/layout clear: hero is the worker, section below is "Selected service". Consider a future provider profile route only when needed.
...
- Affected files/components: `app/worker/[workerId].tsx`, `app/(tabs)/search.tsx`, `app/(tabs)/index.tsx`.
- Risk level: Low.

#### 11. Trust/verification placement needs a product decision

- Problem: The docs agree that verification is a trust gate, but they differ slightly on where public verification cues should appear. The current details do not show verification status at all.
...
- Why it hurts the product: Trust is central, but over-badging clutters screens while under-explaining trust can make the app feel generic.
- Recommended fix: Product owner should choose one detail treatment. Recommended compromise: no repeated feed badges; one concise trust line in the detail trust section when verified data exists.
- Exact replacement copy or layout rule: `Barangay verified` with supporting text only in an expanded trust/safety area: `Verified by barangay review. Private ID documents are never shown.`
- Affected files/components: `app/job/[jobId].tsx`, `app/worker/[workerId].tsx`, card components if the decision changes.
- Risk level: Low.
...
- Evidence from code/UI/source-of-truth: Job Details label at `app/job/[jobId].tsx:329`; Worker Profile label at `app/worker/[workerId].tsx:448`; design system allows clear action labels in `docs/08-design-system.md:135`.
...
- Evidence from code/UI/source-of-truth: Job Details options placeholder at `app/job/[jobId].tsx:65`; Worker Profile placeholder at `app/worker/[workerId].tsx:76`; Search card placeholders at `components/search/SearchJobResultCard.tsx:28` and `components/search/SearchWorkerResultCard.tsx:38`; reports/moderation are planned in `docs/03-feature-map.md:36`.
...
2. Hero
   ...
   - Optional one-time trust cue if product confirms detail verification display.
...
8. Posted by
   ...
   - Optional one-time verification/trust cue if confirmed.
...
- Service inactive/unavailable/messages off: disable CTA with `Messages unavailable`.
...
- If saved items are connected, show `Save` and `Saved`.
- If the user is unverified and save remains gated, route to verification with clear copy.
- If saving is not connected, remove or disable the action. Do not show "later slice" placeholder alerts in production-facing UI.
...
- `Barangay verified`
- `Verified by barangay review. Private ID documents are never shown.`
- `Complete barangay verification to message, save, post, or review.`
...
1. Fix message availability and CTA state
   - Files/components likely affected: `services/conversation.service.ts`, `app/job/[jobId].tsx`, `app/worker/[workerId].tsx`.
...
2. Remove fake trust metrics
   - Files/components likely affected: `services/marketplace.helpers.ts`, `app/worker/[workerId].tsx`, cards/search mappings.
...
3. Decide and implement Save behavior
   - Files/components likely affected: details, Home card, Search cards, saved item service if implemented.
   - What to change: connect `saved_items` or hide/disable Save.
   - Why: visible bookmarks should be real.
   - Manual test: verified save/unsave, unverified gated save, reload persistence.

4. Clean Worker Profile IA
   - Files/components likely affected: `app/worker/[workerId].tsx`.
...
- Should public detail pages show one `Barangay verified` trust cue, or should verification remain implicit because public interaction is already gated?
- Should `Save` be implemented now using `saved_items`, or hidden until the saved-items slice is prioritized?
- Should Job Details ever use `Message to show interest`, or should the button stay simply `Message client`?
- Should service detail remain at `/worker/[serviceId]`, or is a separate provider-level profile route needed later?
...
- Should unavailable-but-active workers still accept messages, or should "unavailable" pause messaging by default?

## Implementation Checklist

- [ ] Confirm ideal information architecture
- [ ] Fix repeated information
- [ ] Improve match card logic
- [ ] Improve Worker Profile details
- [ ] Improve Job Details content
- [ ] Standardize CTA behavior
- [ ] Add/clean edge states

...
| Home feed | `app/(tabs)/index.tsx`, Figma dashboard | `jobs`, `services`, `profiles`, `reviews` | P0 | Partial | Home now loads real open jobs and active services from Supabase, preserves intent-based default filters, and uses lightweight preference scoring plus mixed `For you` ordering. Saved-item polish remains deferred. |
...
| Job browsing | Home/search screens, job detail | `jobs` | P0 | Partial | Providers can view open jobs. Job details are task-first and keep poster trust metrics separate from job tags. `explore` is legacy and should be replaced by Home/Search. |

...
| Provider browsing | Home/search screens, worker detail | `profiles`, `provider_profiles`, `services`, `reviews` | P0 | Partial | Clients can view providers. Worker details are person-first and prioritize rating, jobs done, availability, services, and rate. `explore` is legacy and should be replaced by Home/Search. |

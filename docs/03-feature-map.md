...
| Home feed | `app/(tabs)/index.tsx`, Figma dashboard | `jobs`, `services`, `profiles`, `reviews` | P0 | Partial | Current slice is a mostly demo/static mixed jobs/workers feed using Figma-style `JobCard` and `WorkerCard` components. Preferences only set ordering and the initial filter; verified-origin database filtering is pending. Next: route View Job and View Profile to browse-only detail screens, then replace locked action alerts with consistent verification routing. |

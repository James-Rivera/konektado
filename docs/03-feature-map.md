...
| Profile view/edit | `app/(tabs)/profile.tsx` | `profiles`, `provider_profiles`, `client_profiles`, `credentials`, `reviews`, `saved_items`, drafts | P0 | Partial | Profile dashboard separates Profile Data from Marketplace Activity. Completion is based on identity, capability, preference, trust, and reputation readiness, not active listings. |
| Service listings | Work Profile defaults, offer service screen, profile activity | `services`, `service_drafts`, `provider_profiles` | P0 | Partial | Service creation, owned-service listing, drafts, and active-service search use listing-owned rates, schedules, photos, and message settings. Work Profile only provides blank-draft defaults. |
...
| Provider browsing | Home/search screens, worker detail | `profiles`, `provider_profiles`, `services`, `credentials`, `reviews` | P0 | Partial | Clients can view providers. Worker details are person-first and separate identity/trust/capabilities/reviews from marketplace activity such as active service listings and listing rates. `explore` is legacy and should be replaced by Home/Search. |

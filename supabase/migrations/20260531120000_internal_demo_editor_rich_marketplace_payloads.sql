-- Let the protected internal editor curate the marketplace fields already
-- rendered by Home, Search, and detail screens. Public return shapes stay
...
begin
  ...
  values (
    ...
    coalesce(array(select jsonb_array_elements_text(coalesce(p_payload->'photoUrls', '[]'::jsonb))), array[]::text[]),
    ...
    coalesce((p_payload->>'budgetNegotiable')::boolean, false),
    nullif(trim(coalesce(p_payload->>'workersNeeded', '')), '')::integer,
    nullif(trim(coalesce(p_payload->>'scheduleText', '')), ''),
    nullif(trim(coalesce(p_payload->>'experienceLevel', 'any')), ''),
    coalesce((p_payload->>'certificationRequired')::boolean, false),
    nullif(trim(coalesce(p_payload->>'certificationNote', '')), ''),
    coalesce((p_payload->>'allowMessages')::boolean, true),
    coalesce((p_payload->>'autoReplyEnabled')::boolean, false),
    coalesce((p_payload->>'autoCloseEnabled')::boolean, false),
    v_status
  )
...
begin
  ...
  values (
    ...
    coalesce(array(select jsonb_array_elements_text(coalesce(p_payload->'tags', '[]'::jsonb))), array[]::text[]),
    coalesce(array(select jsonb_array_elements_text(coalesce(p_payload->'photoUrls', '[]'::jsonb))), array[]::text[]),
    nullif(trim(coalesce(p_payload->>'yearsExperience', '')), '')::numeric,
    nullif(trim(coalesce(p_payload->>'availabilityText', '')), ''),
    null,
    nullif(trim(coalesce(p_payload->>'rateMin', '')), '')::numeric,
    nullif(trim(coalesce(p_payload->>'rateMax', '')), '')::numeric,
    nullif(trim(coalesce(p_payload->>'rateType', 'per_service')), ''),
    coalesce((p_payload->>'rateNegotiable')::boolean, false),
    nullif(trim(coalesce(p_payload->>'experienceLevel', 'any')), ''),
    coalesce((p_payload->>'certificationAvailable')::boolean, false),
    nullif(trim(coalesce(p_payload->>'certificationNote', '')), ''),
    null,
    'none',
    nullif(trim(coalesce(p_payload->>'barangay', 'Barangay San Pedro')), ''),
    ...
    coalesce((p_payload->>'allowMessages')::boolean, true),
    coalesce((p_payload->>'autoReplyEnabled')::boolean, false),
    coalesce((p_payload->>'autoPauseEnabled')::boolean, false),
    v_active
  )
  returning * into v_row;
